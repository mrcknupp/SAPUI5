		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("LAYOUT_ID") === oObject.LAYOUT_ID;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].LAYOUT_ID;
									} else {
										sCurrentId = aItemsNotInTheList[0].LAYOUT_ID;
									}
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("LAYOUT_ID"),
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function() {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "layout_Odata (0)"
						}),
						success: function() {
							Opa5.assert.ok(true, "The list header displays 'layout_Odata (0)'");
						},
				...
				theListShouldBeSortedAscendingOnUnitNumber: function() {
					return this.theListShouldBeSortedAscendingOnField("LAYOUT_ID");
				},
				...
				theListShouldHaveAllEntries: function() {
					...
					// retrieve all layout_Odata to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function(oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "layout_Odata (" + iExpectedLength + ")"
								}),
								success: function() {
