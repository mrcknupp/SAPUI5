...
sap.ui.define([
	...
	"sap/ui/Device",
	"com/cpfl/layout/model/formatter",
	"sap/m/MessageBox",
	"com/cpfl/layout/model/grouper",
	...
	return BaseController.extend("com.cpfl.layout.controller.Master", {
		...
		onInit: function() {
			...
			this._oGroupSortState = new GroupSortState(oViewModel, grouper.LAYOUT_ID(this.getResourceBundle()));
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("com.cpfl.layout.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getKey()) {
					case "Filter1":
						aFilters.push(new Filter("LAYOUT_ID", FilterOperator.LE, 100));
						break;
					case "Filter2":
						aFilters.push(new Filter("LAYOUT_ID", FilterOperator.GT, 100));
						break;
					default:
						break;
				}
...
//				if (this._oODataModel.hasPendingChanges() && window.vCreateEntity === false) {
				this._leaveEditPage(fnLeave);
			} else {
//				window.vCreateEntity = false;
				...
				function(mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							LAYOUT_ID: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("LAYOUT_ID"))
						}, true);
					}
				}.bind(this),
				function(mParams) {
		...
		_showDetail: function(oItem) {
			...
			debugger;
			this.getRouter().navTo("object", {
				LAYOUT_ID: encodeURIComponent(oItem.getBindingContext().getProperty("LAYOUT_ID"))
			}, bReplace);
		},
		...
		_findItem: function() {
			...
			if (itemToSelect) {
				...
				debugger;
//				oModel.read("/layoutOData?$top=1" , {
//					success : function(oResponseSucess) {
////						sPath = "/layoutOData" + oResponseSucess.results[0].LAYOUT_ID.
//						debugger;
//						window.alert("Teste");
//					}
//				});				
