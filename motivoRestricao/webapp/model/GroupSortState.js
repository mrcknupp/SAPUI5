sap.ui.define([
	...
	return BaseObject.extend("com.cpfl.motivorestricao.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias com.cpfl.motivorestricao.model.GroupSortState
		 */
		...
		/**
		 * Sorts by NAME, or by RESTRICTION_REASON_ID
		 *
		...
		/**
		 * Groups by RESTRICTION_REASON_ID, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function(sKey) {
			...
			if (sKey === "RESTRICTION_REASON_ID") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "RESTRICTION_REASON_ID");

				aSorters.push(
					new Sorter("RESTRICTION_REASON_ID", false,
