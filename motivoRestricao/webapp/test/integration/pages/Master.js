			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("RESTRICTION_REASON_ID") === oObject.RESTRICTION_REASON_ID;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].RESTRICTION_REASON_ID;
									} else {
										sCurrentId = aItemsNotInTheList[0].RESTRICTION_REASON_ID;
									}
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("RESTRICTION_REASON_ID"),
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function() {
					return this.waitFor({
						...
						success: function() {
							Opa5.assert.ok(true, "The list header displays 'MotivoOdata (0)'");
						},
				...
				theListShouldBeSortedAscendingOnUnitNumber: function() {
					return this.theListShouldBeSortedAscendingOnField("RESTRICTION_REASON_ID");
				},
				...
				theListShouldHaveAllEntries: function() {
					...
					// retrieve all MotivoOdata to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function(oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "MotivoOdata (" + iExpectedLength + ")"
								}),
								success: function() {
