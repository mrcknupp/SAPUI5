sap.ui.define([
	...
	function getFrameUrl(sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("com/cpfl/motivorestricao/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#CadastrodeMotivodeRestrição-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#CadastrodeMotivodeRestrição-display";
		}

		return sUrl + sUrlParameters + sHash;
	}

	return Opa5.extend("com.cpfl.motivorestricao.test.integration.pages.Common", {
		...
		getMockServer: function() {
			return new Promise(function(success) {
				Opa5.getWindow().sap.ui.require(["com/cpfl/motivorestricao/localService/mockserver"], function(mockserver) {
